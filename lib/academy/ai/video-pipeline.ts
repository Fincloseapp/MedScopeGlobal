...
function isPlaceholderAsset(meta: Record<string, unknown>): boolean {
  const url = meta.public_url as string | undefined;
  ...
  return Boolean(
    meta.render_provider === "placeholder" ||
      ...
      (meta.render_status === "ready" &&
        ...
        !meta.tts_audio_url &&
        Boolean(url?.includes("gtv-videos-bucket")))
  );
}
...
export async function retryVideoRender(
  ...
  const patch: Record<string, unknown> = {
    ...
    ...(render.tts_audio_url
      ? {
          ...
          avatar_image_url: render.avatar_image_url,
          lesson_format: "audio_lesson",
          public_url: render.public_url,
          thumbnail_url: render.avatar_image_url,
          pending_external_render: false,
        }
      : render.public_url
        ? { public_url: render.public_url, pending_external_render: render.status === "processing" }
  ...
  return {
    ...
    public_url: render.public_url ?? (patch.public_url as string | undefined),
    lesson_format: render.lesson_format ?? (patch.lesson_format as string | undefined),
...
export async function findPlaceholderVideoAssetIds(limit = 50): Promise<string[]> {
  ...
  for (const row of assets ?? []) {
    ...
    if (isPlaceholderAsset(meta)) ids.push(row.id as string);
