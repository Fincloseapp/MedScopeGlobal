...
import { isOpenAiTtsConfigured, queueOpenAiTtsRender } from "@/lib/academy/ai/video-providers/openai-tts-video";
...
export { isOpenAiTtsConfigured } from "@/lib/academy/ai/video-providers/openai-tts-video";
...
export function getPreferredVideoProvider(): VideoProviderName {
  ...
  if (isOpenAiTtsConfigured()) return "openai_tts";
  return "placeholder";
}
...
export function getVideoProviderChain(): VideoProviderName[] {
  ...
  if (isOpenAiTtsConfigured()) chain.push("openai_tts");
  chain.push("placeholder");
  return chain;
}

/**
 * Provider chain: HeyGen → Synthesia → OpenAI TTS audio lesson → placeholder MP4.
 */
export async function queueExternalVideoRender(input: QueueRenderInput): Promise<QueueRenderResult> {
  ...
  return {
    ...
    message: lastError
      ...
      : "No video API keys — placeholder MP4 retained",
  };
}
