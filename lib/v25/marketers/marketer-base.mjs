...
export async function logMarketerActivityDb(admin, { marketerId, action, details, proposalId }) {
  ...
  const { data, error } = await admin
    ...
    .single();
  if (error) {
    appendLog("v25-marketers.log", `logMarketerActivityDb error: ${error.message}`);
    return null;
  }
  return data;
}
...
export async function logProposal(admin, proposal) {
  ...
  await logMarketerActivityDb(admin, {
    ...
    details: { title: proposal.title, partner_id: proposal.partner_id, priority: proposal.priority },
    proposalId: data.id,
  });
  return data;
}
...
export async function runMarketer({ marketerId, segment, partnerFilter, topics, angles }) {
  ...
  await logMarketerActivityDb(admin, {
    ...
    details: { proposals: proposals.length, trafficScore: hints.trafficScore },
  });
...
export async function discoverPartners(admin, candidates) {
  ...
  if (admin) {
    await logMarketerActivityDb(admin, {
      marketerId: "public",
      action: "partners_discovered",
      details: { count: partners.length, added: candidates.length },
    });
  }
  return partners;
}
