...
/** Nahradí Unsplash/placeholder URL v Supabase v25 SVG obrázky. */
export async function runLegacyImageBackfill(maxItems = 64) {
  ...
  for (const row of rows.slice(0, maxItems)) {
    try {
      ...
      let publicUrl = existing?.publicUrl && !isLegacyImageUrl(existing.publicUrl) ? existing.publicUrl : null;
