...
const MODULE_ICONS = {
  ...
  anatomy: `<circle cx="520" cy="98" r="28" fill="${PALETTE.line}"/><path d="M520 82 v32 M504 98 h32" stroke="${PALETTE.accent}" stroke-width="4" stroke-linecap="round"/>`,
  sleep: `<rect x="488" y="82" width="64" height="36" rx="8" fill="${PALETTE.line}"/><path d="M496 100 h48" stroke="${PALETTE.accent}" stroke-width="3" stroke-linecap="round"/>`,
  food: `<circle cx="520" cy="98" r="28" fill="${PALETTE.line}"/><circle cx="520" cy="98" r="12" fill="${PALETTE.accent}"/>`,
  forest: `<path d="M520 70 L500 118 L540 118 Z" fill="${PALETTE.line}"/><path d="M508 118 v12 M532 118 v12" stroke="${PALETTE.accent}" stroke-width="3"/>`,
  water: `<path d="M520 70 C500 98 500 118 520 118 C540 118 540 98 520 70 Z" fill="${PALETTE.line}"/>`,
...
<svg xmlns="http://www.w3.org/2000/svg" width="800" height="450" viewBox="0 0 800 450" role="img" aria-label="${escapeXml(alt)}">
  ...
  // Brand SVG covers are equipment/diagram illustrations — do not require photo hands directives.
  const style = filterImageStyle(svg, {
    alt,
    prompt: input.prompt,
    imageType,
    requireHandsDirective: false,
  });
...
export function saveGeneratedImage(input) {
  ...
  if (!style.passed) {
    return {
      ok: false,
      error: "style-filter-rejected",
      rejected: style.rejected,
      detail: `style-filter-rejected: ${(style.rejected ?? []).join(", ") || "unknown"}`,
    };
  }
