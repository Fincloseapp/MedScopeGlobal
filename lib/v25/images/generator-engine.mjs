...
export function generateImageSvg(input) {
  ...
  const module = input.module ?? "medicina";
  ...
  if (imageType === "environment") decor = environmentDecor();
  else if (imageType === "object") decor = objectDecor(input.keywords);
  else if (imageType === "icon") decor = iconDecor();

  const icon = MODULE_ICONS[module] ?? MODULE_ICONS.medicina;
...
<svg xmlns="http://www.w3.org/2000/svg" width="800" height="450" viewBox="0 0 800 450" role="img" aria-label="${escapeXml(alt)}">
  ...
  <text x="48" y="72" font-family="system-ui,sans-serif" font-size="13" font-weight="600" fill="${PALETTE.soft}" letter-spacing="0.08em">MEDSCOPE · ${escapeXml(String(input.section).toUpperCase())}</text>
  ...
  ${decor}
  ${icon}
  <text x="48" y="420" font-family="system-ui,sans-serif" font-size="11" fill="${PALETTE.soft}">Neutral · European · Professional · MedScopeGlobal v25.1</text>
</svg>`;

  const style = filterImageStyle(svg, { alt, prompt: input.prompt, imageType });
  return { svg, alt, style, imageType, module };
}
...
export function saveGeneratedImage(input) {
  const { svg, alt, style, imageType, module } = generateImageSvg(input);
  if (!style.passed) {
