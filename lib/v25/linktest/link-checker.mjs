...
export async function runLinkTest(base = BASE) {
  ...
  const registry = readJson("images/registry.json");
  const imageUrls = (registry?.images ?? []).map((i) => i.publicUrl).filter(Boolean);
  let brokenImages = [];
  for (const url of imageUrls.slice(0, 30)) {
    try {
      const res = await fetch(url, { method: "HEAD", headers: { "User-Agent": "MedScopeGlobal-v25.1-linktest" } });
      if (!res.ok) brokenImages.push(url);
    } catch {
      brokenImages.push(url);
    }
  }
  if (brokenImages.length) {
    brokenImages.forEach((u) => appendLog("broken-links.log", `BROKEN IMAGE ${u}`));
    report.brokenImages = brokenImages;
  }

  const state = readJson("v25/system-state.json") ?? {};
  state.tests = {
    ...(state.tests ?? {}),
    linkTest: broken.length === 0 && brokenImages.length === 0 ? "ok" : "fail",
  ...
  return { ok: broken.length === 0 && brokenImages.length === 0, report };
}
