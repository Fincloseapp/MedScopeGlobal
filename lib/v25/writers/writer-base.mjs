...
<p>Nejsme Středomoří s celoroční zeleninou na trhu a dlouhou siestou. Máme zimu, směny, školku, dojíždění a rychlé obědy. Proto u <strong>${topic}</strong> vyhrává adaptace: mražená zelenina, konzervované luštěniny, jednoduché pečené porce a pohyb, který nevyžaduje posilovnu. Cíl není kopírovat Instagram, ale převést principy do českého týdne tak, aby vydržely i v listopadu.</p>`
  ...
  // Third tier — articles that already have week + mini blocks but still sit under magazine length.
  if (
    countPublicArticleWords(html) < PUBLIC_ARTICLE_MIN_WORDS &&
    !/Jak o tématu mluvit v rodině a v práci/i.test(html)
  ) {
    html = insertBeforeSources(
      html,
      `
<h2>Jak o tématu mluvit v rodině a v práci</h2>
<p>U <strong>${topic}</strong> často selhává ne znalost, ale domluva. Partner, děti, kolegové i nadřízený vidí jen výsledek (pozdní příchod, jiné jídlo, telefon v jiné místnosti), ne důvod. Řekněte dopředu jednu větu: co zkoušíte a na jak dlouho. Týdenní experiment se snáze dodrží, když o něm ví i domácnost — nemusí souhlasit se vším, stačí, že vám to nepřekáží.</p>
<p>V práci pomáhá konkrétní, neosobní formulace. Místo „musím se o sebe víc starat“ zkuste „mezi 12:30 a 13:00 neberu chat, jdu na oběd a krátkou procházku“. U směn a home office si označte v kalendáři jeden blok, který není schůzka — i 20 minut se počítá. Pokud pečujete o blízké, střídejte se: jeden hlídá, druhý si ten blok opravdu vezme.</p>
<ul>
<li>Domácnost: jedno společné pravidlo (stůl bez telefonu, společná večeře, večerní klid) vydrží lépe než pět osobních předsevzetí.</li>
<li>Práce: oznamte změnu jako časový blok, ne jako životní styl — kolegové to lépe respektují.</li>
<li>Když to nejde: zmenšete rozsah, ne princip. Deset minut je pořád změna.</li>
</ul>
<h2>Co si odnést do dalšího týdne</h2>
<p>Na konci týdne si napište tři řádky: co šlo samo, co drhlo, co zkusíte příště v menší verzi. Není to deník na celý život — je to pojistka proti pocitu, že „zase nic“. Pokud se téma <strong>${topic}</strong> potkává s chronickým onemocněním, těhotenstvím, léky nebo výraznými příznaky, další krok patří praktickému lékaři, ne dalšímu článku.</p>
<p>Berte text jako mapu pro běžný český týden: škola, směny, MHD, zima, nákup v supermarketu. Cíl není dokonalý režim, ale o něco klidnější rozhodování — a vědět, kdy už je namístě vyšetření místo další rady z internetu.</p>`
    );
  }

  // Fourth tier — leftover stubs that already have week + mini + family blocks.
  if (
    countPublicArticleWords(html) < PUBLIC_ARTICLE_MIN_WORDS &&
    !/Když se týden sype — plán B/i.test(html)
  ) {
    html = insertBeforeSources(
      html,
      `
<h2>Když se týden sype — plán B</h2>
<p>Nemoc v rodině, směna navíc, výpadek hlídání nebo jen únava: u <strong>${topic}</strong> se „ideální týden“ rozpadne dřív, než začne. Připravte si předem menší verzi. Místo hodiny pohybu deset minut chůze po večeři. Místo dokonalého jídelníčku jedno hotové jídlo v lednici. Místo digitálního ticha celý večer jen telefon mimo ložnici. Plán B není selhání — je to způsob, jak se nevrátit na nulu.</p>
<p>Užitečné pravidlo: pokud tři dny po sobě nestíháte původní cíl, nesnažte se ho „doplácet“ o víkendu. Zmenšete ho na polovinu a držte dalších sedm dní. Tělo i hlava lépe snášejí stálý malý rytmus než střídání extrému a výčitek.</p>
<ul>
<li>Nemoc nebo horečka: pauza, tekutiny, praktický lékař při zhoršení — ne dohánění plánu.</li>
<li>Směny a noční práce: posuňte okno (spánek, jídlo, pohyb) podle směny, ne podle kalendáře sousedů.</li>
<li>Cestování a svátky: udržte jednu kotvu (např. pravidelná snídaně nebo večer bez obrazovky), zbytek nechte volnější.</li>
</ul>
<p>Články MedScopeGlobal jsou pro laickou veřejnost. Nejsou diagnóza, jídelníček na míru ani náhrada vyšetření. Pokud máte chronické onemocnění, užíváte léky, jste těhotná nebo se objeví nové příznaky, další krok patří zdravotníkům, kteří znají váš kontext.</p>`
    );
  }
...
Vrať JSON: { "title", "excerpt", "bodyHtml", "keywords", "metaDescription" }`;
  ...
  const groqOk = process.env.GROQ_API_KEY?.trim()?.startsWith("gsk_");
  const openaiOk = process.env.OPENAI_API_KEY?.trim()?.startsWith("sk-");
  const geminiOk = [
    process.env.GEMINI_API_KEY,
    process.env.GOOGLE_AI_API_KEY,
    process.env.GOOGLE_GENERATIVE_AI_API_KEY,
    ...
    .map((k) => k?.trim())
    .some((k) => k && k.length > 20);
  const llmAvailable = Boolean(groqOk || openaiOk || geminiOk);

  // No LLM keys (typical cloud agent): skip rate-limit loops, use template depth only.
  // Mid-length drafts also prefer deterministic sections over slow LLM retries.
  if (!llmAvailable || (priorWords >= 500 && priorWords < minWords)) {
    applyDeterministicDepth();
    if (!llmAvailable || best.wordCount >= minWords) {
      return {
        ...best,
