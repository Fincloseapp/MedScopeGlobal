...
export function fallbackArticle({ topic, topicLabel, seed, writerName }) {
  const title = `${seed}: co stojí za to vědět ještě dnes`;
  return {
    title,
...
export async function generatePublicArticle({ topic, topicLabel, seed, writerName, angle }) {
  const system = `Jsi český zdravotnický redaktor pro širokou veřejnost na medscopeglobal.com (sekce ${topicLabel}).
Piš srozumitelně, přívětivě a marketingově zajímavě — jako kvalitní zdravotnický magazín, ne klinická příručka.
Bez strašení, bez diagnóz — vždy doporuč kontakt s lékařem u nejasných příznaků.
Gramaticky správná čeština, živý jazyk, konkrétní příklady z každodenního života.
Struktura: úvodní hook, 3–5 sekcí s <h2>, odrážky <ul>, závěr s praktickým shrnutím.
