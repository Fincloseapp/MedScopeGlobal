...
async function generatePublicArticleDraft({
  ...
  const authorLabel = `${persona.byline ?? persona.displayName} · ${topicLabel}`;
  const system = `Jsi autonomní český zdravotnický redaktor MedScopeGlobal v26 (sekce ${topicLabel}).
...
Piš jako ${persona.displayName}, ne jako generická AI.
Vrať JSON: { "title": string, "excerpt": string (2–3 věty teaser), "bodyHtml": string (HTML s <p>, <h2>, <ul> nebo <h3>), "keywords": string[], "metaDescription": string }`;
  ...
  base.bodyHtml = appendEditorialByline(base.bodyHtml, persona, topicLabel);
  base.writerPersona = persona.id;
  base.writerDisplayName = persona.displayName;
  base.writerByline = persona.byline ?? persona.displayName;
  return { base, authorLabel, persona };
}
...
export async function generatePublicArticle({
  ...
  for (let attempt = 0; attempt < MAX_SIMILARITY_RETRIES; attempt++) {
    const persona =
      attempt === 0
        ? pickPersonaForArticle(`${topic}:${seed}`, new Date(), writerIndex)
  ...
  return {
    ...
    writerByline: persona.byline ?? persona.displayName,
    hash: topicHash(`${dateTag}:${base.title ?? seed}:${persona.id}`, topic),
    generatedAt: new Date().toISOString(),
    editorialVersion: "26.1",
...
export async function persistPublicArticleToDb(article, bodyHtml) {
  ...
  const row = {
    ...
    source_name: `MedScopeGlobal · ${article.writerByline ?? article.writerDisplayName ?? article.writerName}`,
    meta_description: article.metaDescription ?? article.excerpt?.slice(0, 160),
    ai_generated: true,
    hash_dedup: article.hash,
    metadata: {
      editorial_version: article.editorialVersion ?? "26.1",
      author_persona: article.writerPersona ?? null,
      author_display_name: article.writerDisplayName ?? article.writerName,
      author_byline: article.writerByline ?? article.writerDisplayName ?? article.writerName,
