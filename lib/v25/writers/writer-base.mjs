...
/** Public articles should read as full magazine pieces (~1000–1500 words). */
export const PUBLIC_ARTICLE_MIN_WORDS = 900;
export const PUBLIC_ARTICLE_TARGET_WORDS = 1200;
const softMinWords = (minWords) => Math.min(minWords, 700);
...
Cíl: zajímavý, praktický magazínový text pro laickou veřejnost (ne suchý přehled).
Zachovej strukturu HTML (<p>, <h2>, <h3>, <ul>, včetně <h2>Zdroje</h2>).
Přidej konkrétní příklady z české praxe, týdenní návyky, časté mýty a odpovědi, kdy jít k lékaři.
Bez strašení, bez diagnóz. Piš česky s diakritikou.
Vrať JSON: { "title", "excerpt", "bodyHtml", "keywords", "metaDescription" }`;
