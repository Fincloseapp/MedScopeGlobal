...
async function assignPublicArticleCover(article, dbId) {
  ...
  try {
    ...
    const blob = `${article.title ?? ""} ${article.slug ?? ""} ${imageTopic}`;
    let coverModule = "verejnost";
    if (/spánk|spanek/i.test(blob)) coverModule = "sleep";
    else if (/hydrat|pitn|vod/i.test(blob)) coverModule = "water";
    else if (/výživ|vyziv|jíd|talíř|talir|bílkovin|bilkovin/i.test(blob)) coverModule = "food";
    else if (/stres|dechov/i.test(blob)) coverModule = "forest";
    else if (/rozhovor|podcast/i.test(blob)) coverModule = "interview";
    else if (imageTopic === "nemoci" || imageTopic === "prevence") coverModule = "medicina";
...
export async function runPublicWriters(options = {}) {
  ...
  const report = {
    ...
    editorial: { version: "26.3.0", similarityRejected: 0 },
  };
  ...
  report.ok = report.errors.length === 0 && (report.persisted.db > 0 || report.persisted.files > 0);
