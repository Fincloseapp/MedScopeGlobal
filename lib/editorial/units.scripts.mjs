/**
 * MJS mirror of lib/editorial/units.ts for Node writer/migration scripts.
 * Keep in sync with units.ts — do not rename to units.mjs (conflicts with Next.js resolution).
 */
...
export function publicEditorialByline(locale = "cs") {
...
export function formatEditorialUnitDisplay(_unitId, locale = "cs", _aiAssisted = false) {
  return publicEditorialByline(locale);
}

function detectAiAssisted(article) {
  const safe = article ?? {};
  const meta = safe.metadata && typeof safe.metadata === "object" && !Array.isArray(safe.metadata)
    ? safe.metadata
    : {};
...
export function assignEditorialUnits(article) {
  ...
  if (meta.editorial_unit_primary && EDITORIAL_UNITS[meta.editorial_unit_primary]) {
    return {
      ...
      reviewer:
        meta.editorial_unit_reviewer && EDITORIAL_UNITS[meta.editorial_unit_reviewer]
  ...
  const locale = (safe.locale ?? "cs").toLowerCase();
  const audience = safe.audience ?? "public";
  const rubric = safe.rubric_slug ?? "";
  const level = safe.min_access_level ?? "public";
...
export function buildArticleJsonLdAuthor(assignment, locale = "cs") {
  return {
    "@type": "Organization",
