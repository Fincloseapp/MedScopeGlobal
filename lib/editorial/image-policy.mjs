...
/** Bump when replacing public/assets/covers bytes so CDN/browser caches refresh. */
export const COVER_ASSET_VERSION = "20260829d";
...
export const BANNED_COVER_IDS = [
  ...
  // Same 1559757* photoshoot — sagittal brain model on a rod (was wrongly allowlisted)
...
export const BANNED_STOCK_IDS = [
  ...
  // Doctor-on-phone torso stock (lab coat + smartphone). Never serve as a magazine cover.
...
export const BANNED_VISUAL_KEYWORDS = [
  ...
  /sagittal[\s_-]*(?:section\s+)?brain[\s_-]*model/i,
  ...
  /photo-1559757148-5c350d0d3c56/i,
  /photo-1576091160399/i,
  /plastic[\s_-]*(?:heart|organ)[\s_-]*model/i,
  /anatomical[\s_-]*(?:heart|brain|torso)[\s_-]*model/i,
];

/**
 * Editorial audience preference for CZ/EU magazine stock selection & AI prompts.
 * Regional relevance for Czech readers — not a hate filter.
 */
export const AUDIENCE_GUIDANCE = {
  ...
  minimize:
    "generic diversity-stock medical clichés and non-regional clinic tropes when a European/CZ clinic or equipment scene fits better",
  framing:
    "Choose imagery for regional editorial relevance to Czech and EU readers.",
};

/**
 * Allowlisted first-party magazine covers (bytes must stay editorial-safe).
 * Paths historically used for brain-on-stick / unwanted stock were replaced in place
 * (seniors, vitals, clinical, research, walk, tech) — keep filenames for URL stability.
 */
...
export function withCoverCacheBust(url) {
  if (!url) return url;
  const u = String(url);
  if (!u.startsWith("/assets/covers/") && !u.startsWith("/assets/medscopeglobal") && !u.startsWith("/assets/ai/")) {
    return u;
  }
  if (/[?&]v=/.test(u)) return u;
  return `${u}${u.includes("?") ? "&" : "?"}v=${COVER_ASSET_VERSION}`;
}

function localCover(seed, offset = 0) {
  const i = (hashSeed(seed) + offset) % LOCAL_MAGAZINE_COVERS.length;
  return withCoverCacheBust(LOCAL_MAGAZINE_COVERS[i]);
}

/** Curated safe covers — prefer first-party assets. */
export const SAFE_CURATED_PHOTOS = {
  ...
  interview: "/assets/covers/clinical-3.webp",
  hero: "/assets/covers/produce.webp",
  magazine: "/assets/covers/clinical-3.webp",
...
/** Remote Unsplash is never served as a magazine cover — local allowlist only. */
...
export function isBannedCoverUrl(url) {
  ...
  // Never serve remote Unsplash/Wikimedia as magazine covers — local allowlist only
  if (/unsplash\.com|wikimedia\.org|source\.unsplash/i.test(u)) return true;
  return false;
}

export function isAllowedLocalCover(url) {
  if (!url) return false;
  const path = String(url).split("?")[0];
  if (LAPTOP_BRAND_COVERS.some((c) => path === c || path.endsWith(c))) return true;
  return LOCAL_MAGAZINE_COVERS.some((c) => path === c || path.endsWith(c.replace(/^\//, "/")));
}
...
export function pickLaptopBrandCover(seed) {
  ...
  return withCoverCacheBust(LAPTOP_BRAND_COVERS[i]);
}
...
export function curatedCoverForModule(moduleKey, seed) {
  ...
  if (key === "laptop" || key === "digitalHealth") {
    return withCoverCacheBust(SAFE_CURATED_PHOTOS[key]);
  }
  if (SAFE_CURATED_PHOTOS[key]?.startsWith("/assets/")) {
    return withCoverCacheBust(SAFE_CURATED_PHOTOS[key]);
  }
 ...
 * Banned / remote stock / empty → curated local fallback.
 */
export function resolveEditorialCover(input = {}) {
  ...
  if (direct && !isBannedCoverUrl(direct)) {
    if (isLaptopSceneHint(`${input.title ?? ""} ${input.excerpt ?? ""} ${direct}`)) {
      if (!/medscope/i.test(direct) || /unsplash\.com|wikimedia\.org/i.test(direct)) {
        return pickLaptopBrandCover(seed);
      }
    }
    if (isAllowedLocalCover(direct) || /supabase\.co\/storage\/v1\/object\/public\/media\/v25-images/i.test(direct)) {
      return withCoverCacheBust(direct.startsWith("/assets/") ? direct.split("?")[0] : direct);
    }
    // Unknown remote / non-allowlisted local → force curated
...
export function scoreCoverCandidate({ url, prompt, title, excerpt } = {}) {
  ...
  if (ALLOWED_UNSPLASH_IDS.some((id) => (url ?? "").includes(id))) score += 5;
...
export const EDITORIAL_IMAGE_POLICY_VERSION = "2026.08.29d";
