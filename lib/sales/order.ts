...
import { isSalesPackageId, salesChargeCzk, salesPackageById } from "@/lib/sales/packages";
import type { SalesBillingInterval, SalesContract, SalesProspect } from "@/lib/sales/types";
...
export async function createPausalOrder(input: CreatePausalOrderInput): Promise<{
  ...
  const billingInterval: SalesBillingInterval = input.billingInterval === "year" ? "year" : "month";
  ...
  const billedMonths = billingInterval === "year" ? 12 : 1;
  const end = new Date(addMonthsIso(start, billedMonths));
  const invoice = await issueRetainerInvoice(
    db,
    contract,
    prospect,
    start,
    end,
    salesChargeCzk(pkg.priceCzkMonth, billingInterval)
  );
  if (invoice) {
  ...
  let checkoutUrl: string | null = null;
  try {
    checkoutUrl = await createRetainerCheckoutUrl(contract, prospect, billingInterval);
    if (checkoutUrl) {
