...
function priceVars(locale: string) {
  ...
  return {
    month: formatCzkListPrice(month, locale),
    year: formatCzkListPrice(salesYearlyCzk(month), locale),
    effective: formatCzkListPrice(salesYearlyEffectiveMonthCzk(month), locale),
  };
}
...
function priceLine(locale = "cs"): string {
  const month = salesEntryMonthlyCzk();
  if (locale === "cs") {
    return `${formatSalesCzk(month)} / ${formatSalesCzk(salesYearlyCzk(month))} (${formatSalesCzk(salesYearlyEffectiveMonthCzk(month))}/měs.)`;
  }
  const p = priceVars(locale);
  return `${p.month} / ${p.year} (${p.effective}/mo)`;
}
...
const PACKS: Record<string, Pack> = {
  cs: {
    ...
    intro:
      "ozýváme se z obchodního oddělení MedScopeGlobal (medscopeglobal.com). Tržiště je samostatná B2B část webu — ne magazín a ne oslovení čtenářů. Firmy tam vkládají nabídky a poptávky služeb.",
    marketplace: "Přímý vstup na tržiště:",
    price:
      "Základní měsíční paušál Start je __MONTH__. Roční předplatné má 2 měsíce zdarma (__YEAR__ / rok, __EFFECTIVE__ / měs.). Nejsme plátci DPH. Provizi z obchodu nebereme.",
  ...
  sk: {
    ...
    price:
      "Základný mesačný paušál Start je __MONTH__. Ročné predplatné má 2 mesiace zadarmo (__YEAR__ / rok). Nie sme platcami DPH.",
  ...
  pl: {
    ...
    price:
      "Podstawowy abonament Start: __MONTH__ miesięcznie. Roczna płatność = 2 miesiące gratis (__YEAR__ / rok). Nie jesteśmy płatnikami VAT w CZ.",
  ...
  de: {
    ...
    price:
      "Die monatliche Start-Pauschale beträgt __MONTH__. Jahresabo mit 2 Monaten gratis (__YEAR__ / Jahr). Wir sind in CZ nicht umsatzsteuerpflichtig.",
  ...
  fr: {
    ...
    price:
      "Le forfait mensuel Start est de __MONTH__. L’abonnement annuel offre 2 mois gratuits (__YEAR__ / an). Non assujettis à la TVA en Tchéquie.",
  ...
  it: {
    ...
    price:
      "Il pacchetto mensile Start è __MONTH__. L’abbonamento annuale include 2 mesi gratis (__YEAR__ / anno). Non siamo soggetti IVA in Cechia.",
  ...
  es: {
    ...
    price:
      "El plan mensual Start cuesta __MONTH__. El pago anual incluye 2 meses gratis (__YEAR__ / año). No somos sujetos de IVA en Chequia.",
  ...
  pt: {
    ...
    price:
      "O plano mensal Start custa __MONTH__. A anuidade inclui 2 meses grátis (__YEAR__ / ano). Não somos sujeitos passivos de IVA na Chéquia.",
  ...
  "pt-BR": {
    ...
    price:
      "O plano mensal Start custa __MONTH__. O anual inclui 2 meses grátis (__YEAR__ / ano).",
  ...
  ro: {
    ...
    price:
      "Abonamentul lunar Start este __MONTH__. Plata anuală include 2 luni gratuite (__YEAR__ / an).",
  ...
  hu: {
    ...
    price:
      "A havi Start csomag __MONTH__. Az éves előfizetés 2 hónap ingyenes (__YEAR__ / év).",
  ...
  ru: {
    ...
    price:
      "Базовый ежемесячный тариф Start — __MONTH__. Годовая оплата: 2 месяца бесплатно (__YEAR__ / год).",
  ...
  uk: {
    ...
    price:
      "Базовий місячний тариф Start — __MONTH__. Річна оплата: 2 місяці безкоштовно (__YEAR__ / рік).",
  ...
  be: {
    ...
    price:
      "Базавы месячны тарыф Start — __MONTH__. Гадавая аплата: 2 месяцы бясплатна (__YEAR__ / год).",
  ...
  "zh-CN": {
    ...
    marketplace: "市场直达链接：",
    price: "Start 月费 __MONTH__；年付赠两个月（__YEAR__/年）。",
    cta: "开通套餐并发布：",
  ...
  ja: {
    ...
    price: "Start 月額 __MONTH__。年払いは 2 か月無料（__YEAR__/年）。",
    cta: "プラン開設と掲載：",
  ...
  ko: {
    ...
    marketplace: "마켓 바로가기:",
    price: "Start 월 __MONTH__, 연 결제는 2개월 무료(연 __YEAR__).",
    cta: "요금제 개설 및 게시:",
  ...
  vi: {
    ...
    price: "Gói Start __MONTH__/tháng. Thanh toán năm được tặng 2 tháng (__YEAR__/năm).",
  ...
  id: {
    ...
    price: "Paket bulanan Start __MONTH__. Tahunan termasuk 2 bulan gratis (__YEAR__/tahun).",
  ...
  en: {
    ...
    price:
      "The Start monthly retainer is __MONTH__. Annual billing includes two months free (__YEAR__ / year, __EFFECTIVE__ / month). We are not VAT-registered in Czechia.",
...
export function campaignMarketplaceEmail(input: {
  ...
  const price = fillPrice(pack.price, input.locale);
  const signature = publicBrandSignature(input.locale, entity);
  const html = `
    ...
    <p>${esc(signature)}<br>${esc(entity.supportEmail)}${
  ...
  const text = [
    ...
    `${signature} · ${entity.supportEmail}`,
  ].join("\n\n");
  return { subject, html, text, marketplaceUrl: market, pausalUrl: pausal, priceLine: priceLine(input.locale) };
}
