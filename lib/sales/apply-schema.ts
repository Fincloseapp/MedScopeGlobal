...
export const SALES_DEPARTMENT_SQL = `
...
export async function applySalesDepartmentSchema(): Promise<SchemaApplyResult> {
  if (schemaReady) return { ok: true, skipped: true };
  const outcome = await runManagementQuery(SALES_DEPARTMENT_SQL);
  if (outcome.ok) {
    schemaReady = true;
    return { ok: true };
  }
...
export function markSalesSchemaReady(): void {
  schemaReady = true;
}
