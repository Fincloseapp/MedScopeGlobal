...
import {
  ...
  formatEditorialByline,

} from "./editorial-prompts.mjs";

import { pickEditorialUnitForArticle, auditUnitCoverage } from "./editorial-unit-rotation.mjs";
...
assert(formatEditorialByline(p0, "prevence").includes("MedScopeGlobal"), "persona maps to editorial unit byline");

const unitA = pickEditorialUnitForArticle("prevence:screening", new Date("2026-07-03"), 0);
const unitB = pickEditorialUnitForArticle("nemoci:diabetes", new Date("2026-07-03"), 1);
assert(unitA.primary && unitA.reviewer, "editorial unit rotation assigns primary and reviewer");
assert(unitA.primary !== unitB.primary || unitA.reviewer !== unitB.reviewer, "different seeds yield different units (usually)");

const coverage = auditUnitCoverage(14);
assert(coverage.missing.length === 0, `all ${coverage.total} editorial units in rotation (missing: ${coverage.missing.join(", ")})`);
...
assert(buildTopicFormatPrompt("dlouhovekost").includes("healthspan"), "dlouhovekost format present");
