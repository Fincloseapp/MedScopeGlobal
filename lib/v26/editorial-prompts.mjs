...
- Podpis autora: jméno a role (např. „Klára Horáková, redaktorka zdravotnictví") — ne generická „redakce MedScopeGlobal".`;
...
export function buildPersonaStylePrompt(persona) {
  ...
  const bio = persona.authorBio ? `\nBio autora (volitelně v perexu nebo závěru): ${persona.authorBio}` : "";
...
Podpis autora v textu: ${byline}.${bio}`;
...
export function formatEditorialByline(persona) {
  ...
  return persona?.byline ?? persona?.displayName ?? "Redakce MedScopeGlobal";
...
export function wrapContentInV26Structure({ title, excerpt, bodyHtml, personaName, persona, topic, topicLabel = "Veřejnost" }) {
  ...
  const byline = persona ? formatEditorialByline(persona) : personaName ?? "Redakce MedScopeGlobal";
