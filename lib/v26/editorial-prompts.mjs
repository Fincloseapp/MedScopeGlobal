...
export function buildArticleUserPrompt({ seed, angle, topicLabel, persona, attempt = 0 }) {
  ...
  const lengthHint =
    ...
    persona?.tone === "analytical" || persona?.tone === "investigative"
      ...
      ? `Délka ${PUBLIC_ARTICLE_LENGTH_RANGE} slov (cíl ~${PUBLIC_ARTICLE_TARGET_WORDS}) — hlubší analýza, více dat, kontextu a praktických příkladů z Česka.`

      : persona?.tone === "narrative" || persona?.tone === "empathetic"

        ? `Délka ${PUBLIC_ARTICLE_LENGTH_RANGE} slov (cíl ~${PUBLIC_ARTICLE_TARGET_WORDS}) — bohatší vyprávění, scény, dialogy a konkrétní tipy.`
