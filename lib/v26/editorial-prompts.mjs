...
export function isEnglishDominantTitle(text) {
  ...
  if (CS_CHAR_RE.test(t) || CS_PLAIN_WORD_RE.test(t)) return false;

  if (!CS_CHAR_RE.test(t)) {
...
export function buildForeignMagazineStylePrompt() {
  ...
  return `Stylistický vzor: česká adaptace úspěšných zahraničních zdravotnických médií (BMJ Best Practice, NYT Well, Harvard Health, Healthline) — etická inspirace, ne kopírování.
...
- Citace v textu: u důležitých tvrzení uveď zdroj („podle MZČR", „meta-analýza v BMJ (2024)", „doporučení WHO").
...
export function buildV26StructurePrompt(audience = "public", topic = null) {
  ...
  const diplomaticBlock = audience === "public" ? `\n${buildDiplomaticTonePrompt()}\n${buildReaderBenefitPrompt()}` : "";



  return `Redakční standard MedScope v26.3 — každý článek MUSÍ mít 4–5 sekcí jako <h2>:
...
export function buildArticleUserPrompt({ seed, angle, topicLabel, persona, attempt = 0 }) {
  ...
  const lengthHint =
    ...
    persona?.tone === "analytical" || persona?.tone === "investigative"
      ...
      ? "Délka 1100–1400 slov — hlubší analýza, více dat a kontextu."

      : persona?.tone === "narrative" || persona?.tone === "empathetic"

        ? "Délka 1200–1500 slov — bohatší vyprávění, scény a praktické příklady."

        : "Délka 1000–1300 slov — podrobný, ale srozumitelný text.";
