...
export function buildArticleUserPrompt({ seed, angle, topicLabel, persona, attempt = 0 }) {
  ...
  const lengthHint =
    ...
    persona?.tone === "analytical" || persona?.tone === "investigative"
      ...
      ? "Délka 1300–1600 slov — hlubší analýza, více dat, kontextu a praktických příkladů z Česka."

      : persona?.tone === "narrative" || persona?.tone === "empathetic"

        ? "Délka 1300–1600 slov — bohatší vyprávění, scény, dialogy a konkrétní tipy."

        : "Délka 1200–1500 slov — podrobný, živý magazínový text, ne suchý přehled.";
