...
export async function loadRecentPublicArticles(admin, { maxAgeDays = 21, limit = 120 } = {}) {
  ...
  const { data, error } = await admin
    .from("articles")
    .select("title, excerpt, slug, locale, public_topic, published_at, metadata")
    .eq("audience", "public")
    .eq("published", true)
  ...
  return (data ?? []).map((r) => ({
    ...
    slug: r.slug,
    locale: r.locale,
    public_topic: r.public_topic,
    published_at: r.published_at,
    metadata: r.metadata,
    writer_seed: r.metadata?.writer_seed ?? null,
  }));
}
...
export function loadRecentFromPublicIndex(readJsonFn) {
  try {
    ...
    return articles.slice(0, 80).map((a) => ({
      ...
      slug: a.slug,
      locale: a.locale,
      published_at: a.published_at ?? a.generatedAt,
      writer_seed: a.seed ?? a.writer_seed ?? null,
    }));
  } catch {
    return [];
  }
