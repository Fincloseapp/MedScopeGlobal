...
| Vercel deploy | **Assumed triggered** | Auto-deploy from `main`; live HTML still shows pre-remediation `data-ui-version` (verify dashboard) |
...
1. Confirm Vercel production deployment succeeded for `9099e78` and purge cache if needed.
2. Apply Supabase migrations in production.
3. Re-fetch homepage; expect no `data-ui-version` / `data-ui-build` on `<html>`.
