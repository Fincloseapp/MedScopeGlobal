...
export function PublicLeaderboard({
  ...
  entries: PublicHealthLeaderboardEntry[];
  locale?: string;
}) {
      ...
      {entries.map((entry, i) => (
        ...
        >
          ...
          <span className="flex-1 truncate text-sm font-medium text-[#021d33]">
            {entry.display_name ?? `${chrome.userLabel} ${entry.user_id.slice(0, 6)}`}
          </span>
...
export function PublicLeaderboardCta({ locale }: { locale?: string }) {
  const chrome = getVerejnostChrome(locale);
  return (
    <Link
      href={localizePublicHref("/verejnost/zebricek", locale ?? "cs")}
