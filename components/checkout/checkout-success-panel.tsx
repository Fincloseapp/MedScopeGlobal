...
function editorialSuccessCopy(locale: string) {
  ...
  if (primary === "de") {
    return {
      ...
      opened: "Dieses Gerät hat jetzt Zugang. Auf einem anderen Gerät öffnen Sie die Redaktion über den Link in der E-Mail an Ihre Stripe-Adresse.",
  ...
  if (primary === "fr") {
    return {
      ...
      opened: "Cet appareil a maintenant accès. Sur un autre appareil, ouvrez la rédaction via le lien envoyé à l’e-mail Stripe.",
  ...
  if (primary !== "cs") {
    return {
      ...
      opened: "This device now has access. On another device, open Editorial from the link we send to your Stripe email.",
  ...
  return {
    ...
    opened:
      "Toto zařízení má přístup. Na jiném telefonu nebo počítači otevřete Redakci odkazem v e-mailu na adresu ze Stripe.",
    failed:
