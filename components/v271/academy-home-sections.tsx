...
export async function V272AcademyHomeSections() {
  ...
  let flags: Awaited<ReturnType<typeof getCourseVideoFlags>> = {};
  let prepFlags: Awaited<ReturnType<typeof getCourseVideoFlags>> = {};
  try {
    ...
    flags = await getCourseVideoFlags(courses.map((c) => c.id));
    prepFlags = await getCourseVideoFlags(prepCourses.map((c) => c.id));
  } catch {
    return <AccreditedCmeOverview variant="section" />;
  }
