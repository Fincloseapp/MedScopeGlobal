...
const STEP_ICONS: Record<StepId, typeof Mic> = {
  nahrat: Mic,
  zpracovat: Wand2,
  zkontrolovat: ClipboardCheck,
};
...
export function DokumentaceTutorial({
  ...
  return (
    <section
      ...
      aria-label={`${copy.demoBadge} ${ORDIZAPIS.fullName}`}
    >
      {variant === "full" ? (
        <div
      ...
      <div className={`relative ${variant === "full" ? "p-6 sm:p-8 lg:p-10" : ""}`}>
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-sm leading-6 text-slate-600">{copy.tutorialLead}</p>
          </div>
                  ...
                  <div key={`z-${pulse}`} className="dok-tut-fade flex h-full flex-col justify-center gap-4">
                    <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-white">{copy.preparing}</p>
                    ...
                    <ul className="space-y-2 text-xs text-sky-100/85">
                      {copy.preparingItems.map((item, index) => (
                        <li
                          key={item}
                          className={`rounded-xl bg-white/5 px-3 py-2${index === 2 ? " text-sky-100/50" : ""}`}
                        >
                          {item}
                        </li>
                      ))}
                    </ul>
                    <p className="text-center text-[11px] text-sky-100/60">{copy.preparingFoot}</p>
                  </div>
                ) : null}
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl border border-[#d9e8f4] bg-[#fafcff] p-4">
                ...
                <p className="mt-2 text-sm font-semibold text-[#021d33]">{copy.forPractice}</p>
                <p className="mt-1 text-xs leading-5 text-slate-600">{copy.forPracticeBody}</p>
              </div>
              <div className="rounded-2xl border border-[#d9e8f4] bg-[#fafcff] p-4">
                <Lock className="h-4 w-4 text-[#005B96]" />
                <p className="mt-2 text-sm font-semibold text-[#021d33]">{copy.protectKnowHow}</p>
                <p className="mt-1 text-xs leading-5 text-slate-600">{copy.protectKnowHowBody}</p>
              </div>
            </div>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              >
                <Link
                  href={
                    variant === "compact"
                      ? localizePublicHref("/lekari/dokumentace#stahnout", locale ?? "cs")
                      : "#stahnout"
                  }
                >
                  {copy.downloadQr}
            ...
            <p className="mt-3 text-[11px] leading-4 text-slate-500">
              {copy.accessNote} · {ORDIZAPIS.domain}
            </p>
          </div>
        </div>
      </div>
