/** Client helpers: export OrdiZapis notes as Word-compatible .doc */
...
export function buildOrdiZapisDocBlob(note: string, title = "OrdiZapis zápis"): Blob {
...
export function ordizapisDocFilename(templateId?: string | null): string {
  const stamp = new Date().toISOString().slice(0, 10);
  const tpl = (templateId || "zapis").replace(/[^\w.-]+/g, "_");
  return `OrdiZapis-${tpl}-${stamp}.doc`;
}

export function downloadOrdiZapisDoc(
  note: string,
  opts?: { title?: string; templateId?: string | null }
): void {
  if (!note.trim()) return;
  const title = opts?.title || "OrdiZapis · klinický zápis";
  const blob = buildOrdiZapisDocBlob(note, title);
  const url = URL.createObjectURL(blob);
  const a = document.createElement("a");
  a.href = url;
  a.download = ordizapisDocFilename(opts?.templateId);
  a.rel = "noopener";
...
export async function shareOrdiZapisDoc(
  note: string,
  opts?: { title?: string; templateId?: string | null }
): Promise<"file" | "text" | "copied"> {
  if (!note.trim()) return "text";
  const title = opts?.title || "OrdiZapis zápis";
  const filename = ordizapisDocFilename(opts?.templateId);
  const blob = buildOrdiZapisDocBlob(note, title);
