/* Alias of sw-mediprep.js — keep in sync. Scope /app/priprava (no trailing slash). */
const CACHE_NAME = "msg-mediprep-v7";
const SHELL = [
  "/app/priprava",
  ...
  event.waitUntil(
    caches
      ...
      .then((keys) =>
        Promise.all(
          keys
            .filter((k) => k.startsWith("msg-mediprep") && k !== CACHE_NAME)
...
function isStaticAsset(url) {
  return (
    url.pathname.startsWith("/assets/mediprep/") ||
...
self.addEventListener("fetch", (event) => {
  ...
  /* Only the MeDiprep app shell — do not intercept MeDiktor or the rest of the site */
  if (url.pathname.startsWith("/app/priprava")) {
    event.respondWith(
      fetch(req)
        .then((res) => {
          if (res && res.ok) {
