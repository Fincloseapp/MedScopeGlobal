...
import { listPendingExperts, updateUser } from "@/lib/portal/repository";

export async function GET(request: Request) {
  const user = getSessionUserFromRequest(request);
  if (!user || !hasPermission(user, "admin:verify-experts")) return errorResponse("Přístup odepřen", 403);
  return jsonResponse({ experts: await listPendingExperts() });
}
...
export async function POST(request: Request) {
  ...
  const updated = await updateUser(body.userId, {
