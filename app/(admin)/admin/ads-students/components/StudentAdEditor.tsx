...
const MED_TRACKS = [
  { value: "priprava", label: "Příprava na přijímačky" },
  { value: "studium", label: "Studium medicíny" },
];
...
const TOPICS = [
  ...
  { value: "fyziologie", label: "Fyziologie" },
  { value: "biochemie", label: "Biochemie" },
  { value: "patologie", label: "Patologie" },
  { value: "farmakologie", label: "Farmakologie" },
  { value: "příprava-na-přijímačky", label: "Příprava na přijímačky" },
];
...
export function StudentAdEditor({
  editing,
  onSaved,
  onCancel,
}: {
  editing: StudentAdCampaign | null;
  onSaved: () => void;
  onCancel: () => void;
}) {
  ...
  useEffect(() => {
    if (editing) {
      setForm({
        ...
        study_years: editing.study_years ?? [],
        med_tracks: editing.med_tracks ?? [],
        target_topics: editing.target_topics ?? [],
  ...
  function toggleYear(year: number) {
    setForm((f) => ({
      ...f,
      study_years: f.study_years.includes(year)
        ? f.study_years.filter((y) => y !== year)
        : [...f.study_years, year],
    }));
  }
  ...
  function toggleTrack(track: string) {
    setForm((f) => ({
      ...f,
      med_tracks: f.med_tracks.includes(track)
        ? f.med_tracks.filter((t) => t !== track)
        : [...f.med_tracks, track],
    }));
  }
    ...
    <form onSubmit={handleSubmit} className="grid gap-4 rounded-xl border bg-white p-6 md:grid-cols-2">
      <div className="md:col-span-2">
        <h2 className="font-display text-lg font-semibold text-[#021d33]">
          {editing ? "Upravit studentskou kampaň" : "Nová studentská kampaň"}
        </h2>
        <p className="text-sm text-muted-foreground">
          student_ad_campaigns · targeting ročník, téma, příprava na přijímačky, affiliate
        </p>
      </div>

      <div className="space-y-2 md:col-span-2">
        <Label htmlFor="sad-title">Název</Label>
        <Input
          id="sad-title"
          required
          value={form.title}
          onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))}
          placeholder="Medstart — přípravný kurz"
        />
      </div>

      <div className="space-y-2 md:col-span-2">
        <Label htmlFor="sad-body">HTML obsah</Label>
        <Textarea
          id="sad-body"
          rows={4}
          value={form.body_html}
          onChange={(e) => setForm((f) => ({ ...f, body_html: e.target.value }))}
          placeholder="<p>Krátký popis partnera pro studenty…</p>"
        />
      </div>

      <div className="space-y-2">
        <Label htmlFor="sad-link">Affiliate URL</Label>
        <Input
          id="sad-link"
      ...
      <div className="space-y-2">
        <Label htmlFor="sad-cta">CTA text</Label>
        <Input
          id="sad-cta"
          value={form.cta_text}
      ...
      <div className="space-y-2">
        <Label htmlFor="sad-freq">Frekvence (1–10)</Label>
        <Input
          id="sad-freq"
          type="number"
          min={1}
      ...
      <div className="space-y-2 md:col-span-2">
        <Label>Med track (prázdné = vše)</Label>
          ...
          {MED_TRACKS.map((t) => (
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium transition ${
                form.med_tracks.includes(t.value)
      ...
      <div className="space-y-2 md:col-span-2">
        <Label>Ročníky (prázdné = vše)</Label>
        <div className="flex flex-wrap gap-2">
          {STUDY_YEARS.map((y) => (
            <button
              key={y}
      ...
      <div className="space-y-2 md:col-span-2">
        <Label>Témata (prázdné = vše)</Label>
        <div className="flex flex-wrap gap-2">
          {TOPICS.map((t) => (
            <button
              key={t.value}
