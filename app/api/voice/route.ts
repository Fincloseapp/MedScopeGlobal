...
import { generateVoice } from "@/lib/v40/ai/voice-elevenlabs";
...
export async function POST(request: Request) {
  ...
  const script = (body.script ?? body.text)?.trim();
  if (!script) {
...
export async function GET(request: Request) {
  ...
  const result = await generateVoice({ script: script.trim(), title: url.searchParams.get("title") ?? "voice" });
  return NextResponse.json(
    {
      ok: result.status === "ready",
      voice_provider: result.voice_provider,
