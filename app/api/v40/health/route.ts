...
export async function GET() {
  ...
  const openaiHealth = isOpenAiTtsConfigured() ? await checkOpenAiTtsHealth() : { valid: false, status: 0, detail: "not configured" };
  ...
  return NextResponse.json({
    ...
    subsystems: {
      ...
      v40: {
        ...
        openaiTtsValid: openaiHealth.valid,
        openaiTtsStatus: openaiHealth.status,
    ...
    blockers: !openaiHealth.valid && isOpenAiTtsConfigured()
      ? ["OpenAI TTS probe failed — check OPENAI_API_KEY on Vercel"]
      : [],
    compat: {
      v39: "/api/v39/health",
      v38: "/api/v38/health",
