...
import { MEDIKTOR_APP, appSeoDescription, appSeoTitle } from "@/lib/apps/catalog";
...
export async function generateMetadata(): Promise<Metadata> {
  const base = buildV20PageMetadata({
    title: appSeoTitle(MEDIKTOR_APP),
    description: appSeoDescription(MEDIKTOR_APP),
...
export default function MediktorHubPage() {
    ...
    <div className="bg-[#fafcff]">
      <JsonLdScript
        data={softwareApplicationJsonLd({
          name: MEDIKTOR.shortName,
          description: appSeoDescription(MEDIKTOR_APP),
          url: MEDIKTOR.routes.marketing,
          installUrl: MEDIKTOR_APP.downloadPath,
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
            ...
            <div>
              ...
              <p className="mt-2 text-base font-medium text-sky-100">{hero.tagline}</p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Button asChild size="lg" className="h-12 rounded-full bg-white px-6 text-[#021d33] hover:bg-sky-50">
              <Link href={hero.ctaApp.href}>{hero.ctaApp.label}</Link>
            </Button>
            <Button asChild size="lg" variant="outline" className="h-12 rounded-full border-white/40 bg-transparent px-6 text-white hover:bg-white/10">
              <a href="#stahnout">{hero.ctaPrimary.label}</a>
            </Button>
            <Button asChild size="lg" variant="outline" className="h-12 rounded-full border-white/40 bg-transparent px-6 text-white hover:bg-white/10">
              <a href="#jak-to-funguje">{hero.ctaSecondary.label}</a>
            </Button>
            <Button asChild size="lg" variant="outline" className="h-12 rounded-full border-white/40 bg-transparent px-6 text-white hover:bg-white/10">
              <Link href={hero.ctaPricing.href}>{hero.ctaPricing.label}</Link>
            </Button>
          </div>
          ...
          <div className="mt-8 max-w-xl">
            <AppBrandVisual app={MEDIKTOR_APP} compact className="rounded-2xl ring-1 ring-white/20" />
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {m.capabilities.items.map((item) => {
            ...
            return (
              <div key={item.id} className="rounded-2xl border border-[#cfe1f3] bg-white p-5 shadow-sm">
                <Icon className="h-5 w-5 text-[#005B96]" />
                <h3 className="mt-3 font-display text-lg font-semibold text-[#021d33]">{item.title}</h3>
      ...
      <section id="jak-to-funguje" className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
        <h2 className="font-display text-2xl font-bold text-[#021d33]">{m.howItWorks.title}</h2>
        <p className="mt-2 text-slate-600">{m.howItWorks.summary}</p>
        <ol className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {m.howItWorks.steps.map((step) => (
            <li key={step.n} className="rounded-2xl border border-[#cfe1f3] bg-white p-4">
              <span className="text-xs font-bold uppercase tracking-wider text-[#005B96]">Krok {step.n}</span>
              <h3 className="mt-1 font-semibold text-[#021d33]">{step.title}</h3>
      ...
      <section id="ceny" className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      ...
      <section id="ucet" className="border-y border-[#d9e8f4] bg-[#eef6fb]">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
          <div className="flex items-start gap-3">
            <UserCircle className="h-6 w-6 shrink-0 text-[#005B96]" />
            <div>
              <h2 className="font-display text-xl font-bold text-[#021d33]">{m.account.title}</h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600">{m.account.text}</p>
              <div className="mt-4 flex flex-wrap gap-3">
                {m.account.links.map((link) => (
                  <Button key={link.href} asChild variant="outline" className="rounded-full border-[#cfe1f3]">
      ...
      <section id="navod" className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <div className="flex items-start gap-3 mb-6">
          <BookOpen className="h-6 w-6 shrink-0 text-[#005B96]" />
          <div>
            <h2 className="font-display text-2xl font-bold text-[#021d33]">{m.guide.title}</h2>
            <p className="mt-2 max-w-2xl text-sm text-slate-600">{m.guide.text}</p>
            <Button asChild className="mt-4 rounded-full bg-[#005B96]">
              <Link href={m.guide.href}>{m.guide.linkLabel}</Link>
            </Button>
          </div>
        </div>
      ...
      <section id="faq" className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-display text-2xl font-bold text-[#021d33]">{m.faq.title}</h2>
        </div>
      ...
      <section id="workspace" className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
        <h2 className="mb-6 font-display text-2xl font-bold text-[#021d33]">Pracovní prostor</h2>
      ...
      <section id="pravni" className="border-t border-[#d9e8f4] bg-white">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
          <h2 className="font-display text-xl font-bold text-[#021d33]">{m.legal.title}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-600">{m.legal.intro}</p>
          ...
          <ul className="mt-6 flex flex-wrap gap-4 text-sm">
            {m.legal.links.map((link) => (
              <li key={link.href}>
                <Link href={link.href} className="font-medium text-[#005B96] underline hover:no-underline">
                  {link.label}
                </Link>
              </li>
            ))}
