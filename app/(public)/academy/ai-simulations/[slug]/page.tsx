...
export default async function AcademySimulationDetailPage({ params }: Props) {
  ...
  const scenario = (simulation.scenario_json ?? {}) as ScenarioData;
    ...
    <>
      ...
      <div className="mx-auto max-w-3xl space-y-6 px-4 py-10 sm:px-6">
        ...
        {scenario.steps?.length ? (
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Postup vyšetření</p>
        ...
        {!scenario.chief_complaint && !scenario.steps?.length ? (
          <section className="rounded-xl border border-slate-200 bg-slate-50 p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Scénář (demo)</p>
        ...
        <p className="text-center text-xs text-slate-500">
          Interaktivní rozhodovací větve a AI hodnocení — fáze 4.
        </p>
      </div>
    </>
  );
