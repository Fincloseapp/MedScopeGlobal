...
export default async function MediFlowMarketingPage() {
  const locale = await getServerLocale();
  const copy = getMediflowCopy(locale);
        ...
        >
          ...
          <div className="absolute inset-y-[8%] right-[6%] left-[18%] overflow-hidden rounded-l-[2.5rem] border border-emerald-400/20 border-r-0 bg-[#0c1a24]/75 shadow-[-40px_0_80px_rgba(0,0,0,0.35)] backdrop-blur-[2px]">
            <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
              <span className="font-display text-base font-semibold text-emerald-300">{copy.today}</span>
              <span className="text-[11px] uppercase tracking-[0.22em] text-white/40">{copy.journal}</span>
            </div>
            <ul className="divide-y divide-white/8">
              {copy.diaryRows.map((row) => (
