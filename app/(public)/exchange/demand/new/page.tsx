...
export async function generateMetadata() {
  ...
  return buildLocalizedPageMetadata({
    title: copy.kindDemand,
    description: copy.listingNewLead,
    path: "/exchange/demand/new",
    locale,
  });
}

export default async function ExchangeNewDemandPage() {
  const locale = await getServerLocale();
  const copy = getExchangeCopy(locale);
  return (
    <ModulePageShell eyebrow={copy.kindDemand} title={copy.kindDemand} description={copy.listingNewLead}>
