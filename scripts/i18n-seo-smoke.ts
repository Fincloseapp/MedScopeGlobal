...
import { getVipProtocolsCopy } from "../lib/i18n/vip-protocols-copy";
...
assert.ok(isLocaleRoutingExcluded("/r/ai"));
assert.ok(isLocaleRoutingExcluded("/llms.txt"));
...
assert.ok(getSurfaceCopy("cs").searchNoResults.includes("spánek"));
...
assert.equal(getSurfaceCopy("sk").writersTitle, "Redakčné stoly");
assert.equal(getSurfaceCopy("sk").whyTitle, "Aplikácie v mobile, obsah keď ho potrebujete");
assert.ok(!getSurfaceCopy("sk").whyTitle.includes("Apps on your phone"));
assert.equal(getSurfaceCopy("sk").footer.home, "Domov");
assert.equal(getSurfaceCopy("sk").footer.subscribe, "Predplatné");
assert.equal(getSurfaceCopy("sk").footer.register, "Registrácia");
assert.equal(getSurfaceCopy("sk").footer.apps, "Aplikácie");
assert.equal(getSurfaceCopy("sk").footer.legal, "Právne a kontakt");
assert.equal(getSurfaceCopy("pl").footer.apps, "Aplikacje");
assert.equal(getSurfaceCopy("ja").footer.home, "ホーム");
assert.equal(getSurfaceCopy("ja").footer.subscribe, "購読");
assert.equal(getSurfaceCopy("ja").whyTitle, "スマホのアプリ、必要なときに内容");
assert.equal(getAiAgentBrief("sk").footerLabel, "Pre AI agentov");
assert.equal(getAiAgentBrief("ja").footerLabel, "AIエージェント向け");
assert.ok(!getAiAgentBrief("sk").footerLabel.includes("For AI"));
...
assert.ok(getSurfaceCopy("pl").why[0]?.title.includes("Magazyn"));
assert.ok(!getSurfaceCopy("sk").why[0]?.title.includes("Magazine + app"));
assert.ok(getSurfaceCopy("sk").why[0]?.title.includes("Magazín"));
assert.equal(getSurfaceCopy("sk").b2bTitle, "Pre firmy a inštitúcie");
assert.equal(getSurfaceCopy("ja").todayFallback.includes("今日"), true);
assert.ok(!getSurfaceCopy("sk").footer.audiences.some((item) => item.label.startsWith("Everyone")));
assert.ok(!getSurfaceCopy("sk").trending.some((item) => item.label === "VIP protocols"));
...
assert.equal(newsDesksForLocale("ru")[0]?.label, "Новости");
assert.equal(newsDesksForLocale("ja").find((d) => d.id === "verejnost")?.more, "すべての人向け記事");
assert.ok(!newsDesksForLocale("ja").some((d) => d.more === "Articles for everyone"));
assert.equal(newsDesksForLocale("ja").find((d) => d.id === "verejnost")?.label, "一般向け");
...
assert.equal(getSurfaceCopy("sk").cookieTitle, "Cookies a súkromie");
assert.equal(getSurfaceCopy("ja").cookieTitle, "Cookieとプライバシー");
...
assert.ok(getSubscribeCopy("cs").bannerTitle.includes("Zbytek článku máte hned"));
assert.ok(getSubscribeCopy("cs").bannerTitle.includes("jen za"));
assert.ok(/25/.test(getSubscribeCopy("cs").bannerTitle));
assert.ok(getSubscribeCopy("cs").bannerTitle.includes("Kč"));
assert.ok(/1/.test(getSubscribeCopy("fr").bannerTitle));
assert.ok(/1/.test(getSubscribeCopy("de").bannerTitle));
assert.ok(/1/.test(getSubscribeCopy("en-US").bannerTitle));
assert.ok(!getSubscribeCopy("cs").bannerTitle.includes("{price}"));
assert.ok(getSubscribeCopy("cs").bannerPayCta.includes("Zaplatit"));
assert.ok(/25/.test(getSubscribeCopy("cs").bannerPayCta));
assert.ok(!getSubscribeCopy("cs").bannerPayCta.includes("{price}"));
assert.ok(getSubscribeCopy("fr").bannerPayCta.toLowerCase().includes("payer"));
assert.ok(!/platí hned|platba ihned/i.test(getSubscribeCopy("cs").bannerTitle));
assert.ok(!/billed immediately|is billed/i.test(getSubscribeCopy("en").bannerTitle));
assert.ok(!/sofort berechnet/i.test(getSubscribeCopy("de").bannerTitle));
assert.ok(!/facturée tout de suite/i.test(getSubscribeCopy("fr").bannerTitle));
assert.notEqual(getSubscribeCopy("cs").editorialBadge, getSubscribeCopy("cs").daysFree);
assert.ok(!getSubscribeCopy("cs").editorialBadge.includes("14"));
assert.ok(!getSubscribeCopy("de").bannerLead.includes("14 Tage"));
assert.ok(getSubscribeCopy("cs").bannerLead.toLowerCase().includes("dva měsíce"));
...
assert.ok(!getSubscribeCopy("sk").plans.dokumentace.features.join(" ").includes("14 days free"));
assert.ok(!getSubscribeCopy("ja").priceNoteByApp.mediflow.includes("Editorial"));
...
assert.equal(getRevenueCopy("sk").subscribeHint, "Zrušíte kedykoľvek pred ďalším inkasom.");
assert.ok(!getRevenueCopy("sk").subscribeHint.includes("Cancel any time"));
assert.ok(!getRevenueCopy("ja").subscribeHint.includes("Cancel any time"));
assert.equal(getRevenueCopy("ja").subscribeTitle, "広告なしで読み続ける");
assert.equal(getRevenueCopy("pl").subscribeKicker, "Opcjonalna prenumerata");
...
assert.equal(getMarketingCopy("sk").disclaimerTitle, "Dôležité upozornenie");
assert.ok(!getMarketingCopy("ja").disclaimerBanner.includes("Important notice"));
assert.ok(!getFirmyDeskCopy("sk").title.includes("Advertising on"));
...
assert.equal(getExchangeCopy("de").adsCta, "Anleitung für Inserenten");
...
assert.equal(getInzerceCenikCopy("pl").cta, "Oblicz i wyślij");
assert.ok(!getVipProtocolsCopy("sk").startTrial.includes("14 days free"));
assert.ok(getVipProtocolsCopy("sk").startTrial.includes("14 dní"));
...
assert.equal(getNotFoundCopy("sk").title, "Stránka sa nenašla");
assert.ok(!getNotFoundCopy("ja").title.includes("Page not found"));
...
assert.equal(paymentTiersForUser("en-UK").currency, "usd");
...
assert.equal(convertCzkToCharge(99, "en-UK").currency, "usd");
assert.equal(convertCzkToCharge(99, "ja").currency, "usd");
...
assert.ok(!localizeListedCzk("od 450 Kč", "de").includes("Kč"));
assert.ok(!localizeListedCzk("od 450 Kč", "en-US").includes("Kč"));
assert.ok(!localizeListedCzk("450 CZK", "ja").includes("CZK"));
assert.equal(paymentTiersForUser("pl").currency, "eur");
assert.equal(paymentTiersForUser("hu").currency, "eur");
assert.equal(paymentTiersForUser("ja").currency, "usd");
assert.equal(paymentTiersForUser("pt-BR").currency, "usd");
...
assert.ok(convertCzkToCharge(5000, "en-UK").currency === "usd");
...
assert.equal(getRevenueCopy("sk").partnerCta, "Rezervovať inzerciu");
assert.ok(!getRevenueCopy("sk").partnerTitle.includes("This slot is available"));
assert.ok(!getRevenueCopy("ja").mediaKitCta.includes("Book a campaign"));
...
assert.equal(getFirmyDeskCopy("fr").rooms.kosmetika.title.length > 0, true);
assert.ok(!looksLikeCzech(getFirmyDeskCopy("fr").rooms.kosmetika.title));
assert.ok(!looksLikeCzech(getFirmyDeskCopy("de").rooms.kosmetika.title));
...
assert.ok(!looksLikeCzech(getVerejnostChrome("fr").transcriptTitle));
assert.ok(!looksLikeCzech(getVerejnostChrome("de").legalNote));
assert.ok(!looksLikeCzech(getVerejnostChrome("en").videoOverlayCta));
assert.ok(!looksLikeCzech(getVerejnostChrome("fr").slideshowPlay));
assert.ok(!looksLikeCzech(getVerejnostChrome("de").slideshowFooter));
...
assert.equal(getOdborneHubCopy("de").desk.kategorie.title, "Kategorien");
assert.ok(!looksLikeCzech(getOdborneHubCopy("fr").desk.ai.lead));
assert.ok(!looksLikeCzech(getOdborneHubCopy("fr").back));
...
assert.ok(!looksLikeCzech(getAiAssistantCopy("fr").publicTitle));
assert.ok(!getAiAssistantCopy("de").cards.some((card) => card.href.includes("/student")));
assert.ok(getAiAssistantCopy("cs").cards.some((card) => card.href.includes("/student")));
assert.ok(!looksLikeCzech(getLekyHubCopy("fr").title));
