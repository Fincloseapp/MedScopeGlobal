...
if (v28Health?.ok) {
  try {
    ...
    if (!String(json.version).includes("28") && !String(json.version).includes("29")) {
      healthChecks.push({ ok: false, msg: `v28 health version ${json.version} !== 28.x / 29.x` });
    } else {
    ...
    if (!json.features?.includes("stripe-webhook-v28.2") && !json.features?.includes("stripe-webhook-v29")) {
      healthChecks.push({ ok: false, msg: "v28 health missing stripe-webhook feature flag" });
    } else {
      healthChecks.push({ ok: true, msg: "stripe-webhook feature flag" });
    }
    if (json.compat?.v29Health) {
      healthChecks.push({ ok: true, msg: "v28 health v29 compat link" });
    }
  } catch (e) {
...
if (webhookStatus === 400) {
  ...
  webhookChecks.push({ ok: false, msg: `webhook unexpected status ${webhookStatus} (expected 400)` });
}
