...
function verifyLocalApiRoutes() {
  const missing = REQUIRED_API_ROUTES.filter((rel) => !existsSync(join(root, rel)));
  if (missing.length) {
    throw new Error(
      `Missing local API routes (deploy would break cron jobs):\n${missing.map((m) => `  - ${m}`).join("\n")}`
    );
  }
  log(`✓ ${REQUIRED_API_ROUTES.length} critical API routes present locally`);
}
...
function loadEnv() {
  const envPath =
    ...
    (existsSync(join(root, ".env.local")) ? join(root, ".env.local") : join(root, ".env"));
  const fileEnv = {
    ...loadEnvFile(envPath),
...
/** Inject loaded env into process.env for child scripts (verify-env, migrations, etc.). */
...
async function main() {
  ...
  let predeploy = runPredeploy({ SKIP_PREDEPLOY_TYPECHECK: "1" });

  verifyLocalApiRoutes();
