/**
 * One-off edge worker — apply B2B Exchange SQL using production Worker secrets.
 * Run: pnpm db:edge-apply-exchange
 * Then: curl http://127.0.0.1:8788/
 */
import { MIGRATIONS } from "./edge-apply-exchange-data.mjs";
  ...
  async fetch(_request, env) {
    ...
    for (const migration of MIGRATIONS) {
      try {
        ...
        if (/already exists|duplicate key|duplicate_object|relation .* already exists|column .* already exists/i.test(msg)) {
